...
class RungeKuttaExplanation(Scene):
    ...
    def show_numerical_solution(self):
        ...
        # 解析解のグラフ（薄く表示）
        true_graph = axes.plot(lambda x: np.exp(x), x_range=[0, 1], color=GRAY)
        ...
        # 数値計算のパラメータ（脚本に合わせて0.1刻み）
        h = 0.1  # 刻み幅
        x_vals = [0]
        f_vals = [1.0]  # 初期条件 f(0) = 1

        # スタート地点の説明
        start_text = Text("スタート地点: x = 0 のとき f = 1", font_size=28, color=YELLOW)
        start_text.to_edge(UP)
        ...
        # 刻み幅の説明に切り替え
        step_text = Text("0.1 刻みで少しずつ進む", font_size=28, color=YELLOW)
        step_text.to_edge(UP)
        self.play(Transform(start_text, step_text))

        # ルンゲ・クッタ法（4次）でステップを進める
        dots = [current_dot]
        lines = []

        # x = 0.1, 0.2, ... と進む（10ステップ）
        for i in range(10):
            x = x_vals[-1]
            f = f_vals[-1]
            ...
            if i < 3:
                ...
                self.play(Create(arrow), Write(slope_label), run_time=0.4)
                self.wait(0.2)
                ...
                # x = 0.1, 0.2 の座標を表示
                coord_label = MathTex(
                    rf"x = {x_new:.1f}", font_size=20, color=RED
                )
                ...
                new_dot = Dot(axes.c2p(x_new, f_new), color=RED, radius=0.06)
                self.play(Create(line), Create(new_dot), run_time=0.15)
        ...
        # 結果の説明
        result_text = Text("これを繰り返すと関数の形が見えてくる", font_size=28, color=GREEN)
